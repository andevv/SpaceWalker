...
final class SpaceRepository {
    ...
    func fetchMySpaces() -> Single<[JoinedSpace]> {
        let single: Single<JoinedSpacesResponse> = NetworkManager.shared.request(
            "/api/v1/space/my-space",
            method: .get,
            parameters: nil,
            requiresAuth: true
        )
        return single.map { res in
            res.spaces.compactMap { dto in
                guard let joinedAt = ISO8601DateFormatter().date(from: dto.joinedAt) else { return nil }
                return JoinedSpace(id: dto.spaceId, name: dto.name, joinedAt: joinedAt)
            }
        }
    }
